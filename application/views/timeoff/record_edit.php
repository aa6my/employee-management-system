                ...
                <form action="timeoff/save_record" method="POST" id="save_record">
                    ...
                    <div class="col-lg-6" style="padding-left: 0;">
                        <div class="form-group has-feedback">
                            ...
                            <input type="text" name="start_time" id="start_time" class="form-control required datetimepicker" data-date-format="<?= $this->config->item('js_month_format').' '.$this->config->item('js_time_format')?>" value="<?= date($this->config->item('date_format').' '.$this->config->item('time_format'),strtotime($record['start_time']))?>">
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="form-group has-feedback">
                            <label class="end_time" class="control-label"><?= $this->lang->line('End time')?><sup class="mandatory">*</sup></label>
                            <input type="text" name="end_time" id="end_time" class="form-control required datetimepicker" data-date-format="<?= $this->config->item('js_month_format').' '.$this->config->item('js_time_format')?>" value="<?= date($this->config->item('date_format').' '.$this->config->item('time_format'),strtotime($record['end_time']))?>">
                        </div>
                    </div>
                    <div class="clearfix"></div>
                    <div class="col-lg-6" style="padding-left: 0;">
